...
# Confirm the project name (should be "postaryx-prod")
docker compose ls
...
> silently creates or targets a *different, empty* stack. This server's project
> is `postaryx-prod`, so every command must say `-p postaryx-prod`.
...
| App up but shows the *old* version | Pulled but didn't recreate the container | `docker compose -p postaryx-prod up -d postaryx` again; verify with `docker exec postaryx printenv NEXT_PUBLIC_VERSION` |
