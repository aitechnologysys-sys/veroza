...
```bash
# prod-local stack must be running: docker compose -p postaryx-prod up -d
docker run -d --rm --name pg-bridge \
  --network postaryx-prod_postaryx-network \
  -p 127.0.0.1:5433:5433 \
  alpine/socat tcp-listen:5433,fork,reuseaddr tcp-connect:postaryx-postgres:5432
```
...
|---|---|---|
| `postaryx-postgres` | `5432:5432` | **database**, credentials in the public repo |
| `postaryx-redis` | `6379:6379` | **Redis, no password at all** |
| `postaryx-pg-admin` | `8082:80` | **pgAdmin web UI** |
| `postaryx-redisinsight` | `5540:5540` | **RedisInsight web UI** |
