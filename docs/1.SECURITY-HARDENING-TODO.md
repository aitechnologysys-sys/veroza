...
| Host | `127.0.0.1` |
| Port | `5433` |
| Database | `postaryx-db-prod` |
| Username | `postaryx-user` |
| Password | `postaryx-password` |
...
| Database | `postaryx-db-local` | `postaryx-db-prod` |
| User / password | `postaryx-local` / `postaryx-local-pwd` | `postaryx-user` / `postaryx-password` |
...
```yaml
POSTGRES_PASSWORD: postaryx-password    # line 109 — the app database
...
host port, so they are only reachable from inside the `postaryx-network` bridge.
...
postaryx-postgres:
  environment:
    ...
    POSTGRES_USER: ${POSTGRES_USER:-postaryx-user}
    POSTGRES_DB: ${POSTGRES_DB:-postaryx-db-prod}
```
...
(already gitignored) and update `DATABASE_URL` in the `postaryx` service to match.
...
`postaryx-redis` currently has **no `requirepass`, no `maxmemory`, and no eviction
...
Then update `REDIS_URL` in the `postaryx` service to
`redis://:${REDIS_PASSWORD}@postaryx-redis:6379`. Restart and confirm the app still
queues posts — a wrong URL fails at runtime, not at boot.

Watch actual usage before trusting 256 MB:

```bash
docker exec postaryx-redis redis-cli -a "$REDIS_PASSWORD" info memory | grep used_memory_human
```
