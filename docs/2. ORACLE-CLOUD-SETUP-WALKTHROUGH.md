...
2. **Delete the `temporal-ui` and `temporal-admin-tools` service blocks** —
   ...
   need to debug:
   ```bash
   docker compose -p postaryx-prod run --rm temporal-admin-tools tctl --namespace default namespace describe
   ```
...
docker compose -p postaryx-prod logs -f postaryx     # watch nginx + pm2 + prisma-db-push start (~90s)
```
...
1. `docker compose -p postaryx-prod ps` → every container `healthy`/`running`.
...
7. `docker compose -p postaryx-prod logs postaryx | grep -i prisma` → migration ran
   clean, no errors.
