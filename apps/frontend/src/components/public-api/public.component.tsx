...
const getMcpConfig = (
  ...
  if (method === 'path') {
    switch (client) {
      case 'Claude Code':
        return {
          config: `claude mcp add postaryx --transport http "${urlWithKey}"`,
          hint: 'Run this command in your terminal.',
        };
      case 'Cursor':
        return {
          config: json({ mcpServers: { postaryx: { url: urlWithKey } } }),
          hint: 'Add to .cursor/mcp.json in your project root.',
        };
      case 'VS Code / Copilot':
        return {
          config: json({
            servers: { postaryx: { type: 'http', url: urlWithKey } },
          }),
      ...
      case 'Windsurf':
        return {
          config: json({
            mcpServers: { postaryx: { serverUrl: urlWithKey } },
          }),
          hint: 'Add to ~/.codeium/windsurf/mcp_config.json',
        };
      case 'Amp':
        return {
          config: `amp mcp add postaryx ${urlWithKey}`,
          hint: 'Run this command in your terminal.',
        };
      case 'Codex':
        return {
          config: `# ~/.codex/config.toml\n\n[mcp_servers.postaryx]\nurl = "${urlWithKey}"`,
      ...
      case 'Warp':
        return {
          config: json({ postaryx: { url: urlWithKey } }),
  ...
  switch (client) {
    case 'Claude Code':
      return {
        config: `claude mcp add postaryx \\\n  --transport http \\\n  --header "Authorization: ${bearer}" \\\n  "${urlBase}"`,
        hint: 'Run this command in your terminal.',
      };
    case 'Cursor':
      return {
        config: json({
          mcpServers: {
            postaryx: { url: urlBase, headers: { Authorization: bearer } },
          },
        }),
        hint: 'Add to .cursor/mcp.json in your project root.',
      };
    ...
    <div className="bg-newBgColorInnerInner rounded-[12px] border border-newBorder overflow-hidden">
      <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
        ...
        <div className="flex gap-[6px] shrink-0 pt-[2px]">
          <a
            className="cursor-pointer px-[16px] h-[36px] bg-btnPrimary hover:opacity-90 text-white transition-colors rounded-[8px] text-[13px] font-[600] flex items-center gap-[6px]"
...
const localCliSteps = [
  {
    ...
    code: 'npm install -g postaryx',
  },
  {
    label: 'Run: postaryx auth:login',
    code: 'postaryx auth:login',
  },
  {
    label: 'Install the Postaryx skill for your AI agent',
    code: 'npx skills add postaryx/postiz-agent',
  },
] as const;
...
const ciCliSteps = [
  ...
  },
  {
    ...
    code: 'export POSTARYX_API_KEY="{API_KEY}"',
  },
  {
    label: 'Install the Postaryx skill for your AI agent',
    code: 'npx skills add postaryx/postiz-agent',
  ...
  return (
    <div className="flex flex-col gap-[40px]">
      <div className="text-[14px] text-textColor leading-[1.7]">
        ...
        {t(
          'api_auth_note_line2',
          'If you are building a product that schedules posts on behalf of other Postaryx users,'
        )}
        <br />
        {t(
          'api_auth_note_line3',
      ...
      <div className="bg-newBgColorInnerInner rounded-[12px] border border-newBorder overflow-hidden">
        <div className="bg-newBgColorInner px-[20px] py-[14px] border-b border-newBorder flex items-start justify-between gap-[12px]">
          ...
          <div className="flex gap-[6px] shrink-0 pt-[2px]">
            ...
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 13v6a2 2 0 01-2 2H5a2 2 0 01-2-2V8a2 2 0 012-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg>
              {t('n8n_node', 'N8N Node (Postiz-compatible)')}
            </a>
          </div>
        </div>
        <div className="p-[20px] flex flex-col gap-[16px]">
